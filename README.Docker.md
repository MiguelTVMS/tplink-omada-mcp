...
| `getTrafficDistribution`         | Gets traffic distribution by protocol/app type over a time range. Requires `start` and `end` timestamps (ms). |
| `getRetryAndDroppedRate`         | Gets wireless retry rate and dropped packet rate over a time range. Requires `start` and `end` timestamps (ms). |
| `getIspLoad`                     | Gets per-WAN ISP link load over a time range. Requires `start` and `end` timestamps (ms). |
